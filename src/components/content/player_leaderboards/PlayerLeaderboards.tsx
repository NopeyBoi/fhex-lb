...
const PlayerLeaderboards = ({ update }: Props) => {
  ...
  for (let i = 0; i < 1000; i++) {
    if (sortedData[i].username.toLowerCase().includes(inputText)) {
      entries.push(<LeaderboardEntry key={sortedData[i].uuid} position={i + 1} username={sortedData[i].username} pp={sortedData[i].pp} onClick={() => update(sortedData[i].username)}></LeaderboardEntry>);
    }
  }
